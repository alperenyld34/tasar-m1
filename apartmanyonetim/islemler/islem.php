...
//daire Ekleme Bölümü
if (isset($_POST['daireekle'])) {
  if (yetkikontrol()!="yetkili" AND !$api) {
    header("location:../index.php");
    exit;
  }

  $daireekle=$db->prepare("INSERT INTO daire SET
   daire_sakini=:baslik,
   daire_detay=:detay,
   daire_teslim_tarihi=:teslim_tarihi,
   daire_durum=:durum,
   daire_no=:daire_no,
   yuzde=:yuzde,
   daire_baslama_tarihi=:daire_baslama_tarihi
   ");

  $ekleme=$daireekle->execute(array(
   'baslik' => $_POST['daire_sakini'],
   'detay' => $_POST['daire_detay'],
   'teslim_tarihi' => $_POST['daire_teslim_tarihi'],
   'durum' => $_POST['daire_durum'],
   'daire_no' => $_POST['daire_no'],
   'yuzde' => $_POST['yuzde'],
   'daire_baslama_tarihi' => $_POST['daire_baslama_tarihi']
 ));

  if ($_FILES['daire_dosya']['error']=="0") {
    $yuklemeklasoru = '../dosyalar';
    @$gecici_isim = $_FILES['daire_dosya']["tmp_name"];
    @$dosya_ismi = $_FILES['daire_dosya']["name"];
    $benzersizsayi1=rand(100000,999999);
    $isim=tum_bosluk_sil($benzersizsayi1.$dosya_ismi);
    @move_uploaded_file($gecici_isim, "$yuklemeklasoru/$isim");   
    $son_eklenen_id=$db->lastInsertId();
    $dosyayukleme=$db->prepare("UPDATE daire SET
     dosya_yolu=:dosya_yolu WHERE daire_id=:daire_id ");

    $yukleme=$dosyayukleme->execute(array(
     'dosya_yolu' => $isim,
     'daire_id' => $son_eklenen_id
   ));
  }
  ...
  if ($ekleme) {
    ...
    if ($api) {
     ...
     header("location:../daireler?durum=ok");
   }
  ...
  if ($api) {
    echo json_encode(['durum' => 'no','mesaj' => 'İşlem Başarısız', 'hata' => implode(",", $daireekle->errorInfo())]);
  } else {
   header("location:../daireler?durum=no");
 }
 exit;
}
exit;
...
if (isset($_POST['kiraguncelle'])) {
  if (yetkikontrol()!="yetkili" AND !$api) {
    header("location:../index.php");
    exit;
  }

  $kiraguncelle=$db->prepare("UPDATE kira SET
   
	daire_kira=:kira
	kira_tarihi=:kira_tarihi
    kira_durum=:kira_durum
	
   
    WHERE kira_id={$_POST['kira_id']}");

  $guncelle=$kiraguncelle->execute(array(
    'kira' => $_POST['daire_kira'],
    'ira_tarihi' => $_POST['ira_tarihi'],
	'kira_durum' => $_POST['kira_durum'],
   
    
  ));
  if ($_FILES['kira_dosya']['error']=="0") {

    $yuklemeklasoru = '../dosyalar';
    @$gecici_isim = $_FILES['kira_dosya']["tmp_name"];
    @$dosya_ismi = $_FILES['kira_dosya']["name"];
    $benzersizsayi1=rand(10,1000);
    $isim=tum_bosluk_sil($benzersizsayi1.$dosya_ismi);
    @move_uploaded_file($gecici_isim, "$yuklemeklasoru/$isim");   

    $dosyayukleme=$db->prepare("UPDATE kira SET
      dosya_yolu=:dosya_yolu WHERE kira_id=:kira_id ");
  ...
  if ($guncelle) {
   ...
   if ($api) {
     ...
     header("location:../kiralar?durum=ok");
   }
   ...
   if ($api) {
    echo json_encode(['durum' => 'no','mesaj' => 'İşlem Başarısız', 'hata' => implode(",", $kiraguncelle->errorInfo())]);
  } else {
   header("location:../kiralar?durum=no");
 }
 exit;
}
exit;
...
//kira Ekleme Bölümü
if (isset($_POST['kiraekle'])) {
  if (yetkikontrol()!="yetkili" AND !$api) {
    header("location:../index.php");
    exit;
  }

  $kiraekle=$db->prepare("INSERT INTO kira SET
	daire_kira=:kira
	kira_tarihi=:kira_tarihi
    kira_durum=:kira_durum
  ...
  if ($api) {
    echo json_encode(['durum' => 'no','mesaj' => 'İşlem Başarısız', 'hata' => implode(",", $kiraekle->errorInfo())]);
  } else {
   header("location:../kiralar?durum=no");
 }
 exit;
...
if (isset($_POST['kiraguncelle'])) {
  ...
  $kiraguncelle=$db->prepare("UPDATE kira SET
   daire_kira=:kira
    ...
    yuzde=:yuzde WHERE kira_id={$_POST['kira_id']}");
...
if (isset($_POST['profilguncelle'])) {
  ...
  if ($api) {
    $_SESSION['kul_id']=guvenlik($_SESSION['kul_id']);
  }
...
if (isset($_POST['dairesilme'])) {
  if (yetkikontrol()!="yetkili" AND !$api) {
    header("location:../index.php");
    exit;
  }
  $sil=$db->prepare("DELETE from daire where daire_id=:id");
  $kontrol=$sil->execute(array(
...
if (isset($_POST['daireleri_getir'])) {
  if(!$api){
    ...
    $x=$db->prepare("SELECT * FROM daire $order $limit");
    $x->execute();
    $sonuc=$x->fetchAll(PDO::FETCH_ASSOC);
    echo json_encode(['durum' => 'ok', 'daireler' => $sonuc],JSON_NUMERIC_CHECK | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
